...
$query = "SELECT username,userid FROM users WHERE Playing='1' AND userid !='$userid' LIMIT 6";
...
if($row){
    ...
    for($i=0;$i<$c;$i++)
    {
        $ar=array("user"=>$row[$i]["username"],"Id"=>$row[$i]["userid"],"pic"=>$row[$i]["pic"]);
        array_push($Datan,$ar);
    ...
    if($c<6)
    {
        ...
        {
            $ii=$i+10001;
            $ar=array("user"=>"Guest {$ii}","Id"=>$ii,"pic"=>3);
            array_push($Datan,$ar);
            $totalPlayers++;
        }
